...
class Repository(models.Model):
    ...
    path = models.FilePathField(
        ...
        allow_folders=True,
        verbose_name=_("Path"),
        path=settings.LOCAL_BACKUP_PATH,
    )
    sudo = models.BooleanField(
        default=False, help_text=_("Repository needs admin privileges")
    )
...
class Journal(models.Model):
    ...
    action = models.CharField(
        max_length=2, choices=ACTION_CHOICES, verbose_name=_("Action")
    )
    repo = models.ForeignKey(
        Repository, on_delete=models.DO_NOTHING, verbose_name=_("Repository")
    )
